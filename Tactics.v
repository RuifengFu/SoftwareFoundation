...
(** **** 练习：3 星, standard (apply_exercise1) 
    ...
    记住 [Search] 是你的朋友。） *)
Lemma rev_single_reflex : forall (l l' : list nat) ,
  rev l = rev l' -> l = l'.
...
Proof.
  intros l l' H.
  apply rev_single_reflex. rewrite -> rev_involutive. symmetry.
  apply H. Qed.
(** [] *)
...
Proof.
  ...
  rewrite -> H1. symmetry. 
  assert (H3: y :: l = z :: l). { rewrite -> H2. rewrite -> H. reflexivity. }
  injection H3 as H3. apply H3. Qed.
(** [] *)
...
Proof.
  ...
  - intros m. destruct m as [ | m'].
    + simpl. intros H. discriminate H.
    + simpl. intros H. f_equal. apply IHn'. apply H. Qed.
(** [] *)
...
Proof.
  ...
  - intros m. destruct m as [ | m'].
    ...
    + intros H. rewrite <- plus_n_Sm in H. rewrite <- plus_n_Sm in H. 
      simpl in H. injection H as eq. f_equal. apply IHn'. apply eq. Qed.
(** [] *)
...
Proof.
  ...
  - destruct n as [ | n'].
    + intros H. discriminate.
    + intros H. apply IHl'. injection H as H'. apply H'. Qed.
(** [] *)
...
(*   generalize dependent l1.
  ...
  - simpl. intros H. injection H as eq1 eq2. 
    rewrite <- eq1. rewrite <- eq2. reflexivity.
(*   - intros l1 l2. destruct l' as [].
    + destruct p'. simpl. intros H. injection H as eq1 eq2.
      rewrite <- eq1. rewrite <- eq2. reflexivity.
    + destruct p'. simpl. intros H. *)
  - destruct (split (p' :: l')) eqn: E1. intros H.
...
Proof.
  ...
  - intros l1 l2 H. injection H as eq1 eq2. rewrite <- eq1. reflexivity.
  - intros l1 l2. simpl. destruct (split l') as [lx ly] eqn: E. 
    intros H. injection H as eq1 eq2. rewrite <- eq1. rewrite <- eq2.
    simpl. assert (H1: combine lx ly = l'). 
...
(* eqb_true: forall n m : nat, (n =? m) = true -> n = m *)
  ...
  rewrite <- H1 in H2.
  rewrite -> H2. rewrite <- eqb_refl. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - intros l1 l2 H. destruct l1 eqn: E. 
    + symmetry in H. simpl in H. unfold length in H.
      destruct l2 eqn: E2. 
      ++ reflexivity.
      ++ discriminate.
    + intros H1.  simpl in H1. 
      ...
      ++ discriminate.
      ++ discriminate.
  - intros l1 l2 H H1. destruct l1 as [|x0 l1'] eqn:E1.
    + simpl in H1. discriminate.
    + destruct l2 as [|y0 l2'] eqn:E2.
      ++ simpl in H. discriminate.
      ++ unfold combine in H1. injection H1 as eq1 eq2 eq3.
         apply IHl' in eq3. unfold split in eq3. unfold split.
         rewrite -> eq3. rewrite -> eq1. rewrite -> eq2. reflexivity.
         simpl in H. injection H as H2. apply H2. Qed.
...
Proof.
  ...
  - simpl. destruct (test x0) eqn: E.
    + intros H. injection H as eq1 eq2. rewrite <- eq1. apply E.
    + apply IHl'. Qed.
(** [] *)
...
Proof. 
  ...
  + simpl. rewrite -> IHl'. destruct (test x) eqn: E.
    - simpl. unfold existsb'. unfold forallb. rewrite -> E. reflexivity.
    - simpl. unfold existsb'. unfold forallb. rewrite -> E. reflexivity. 
Qed.
