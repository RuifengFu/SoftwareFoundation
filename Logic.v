...
Proof.
  ...
  intros G H. intros .
  apply G in H0. apply H in H0. apply H0.
Qed.
...
Proof.
  intros P. unfold not.
  intros [H0 H1]. apply H1 in H0. apply H0.
Qed.
(** [] *)
...
Proof.
  ...
  - intros [ HP | HQ]. 
    + destruct HP as [x H]. exists x. left. apply H.
    + destruct HQ as [x H]. exists x. right. apply H.
Qed.
 (** [] *)
...
Lemma All_In :
  forall T (P : T -> Prop) (l : list T),
    ...
    All P l.
Proof.
  intros T P l. induction l as [ | x' l' IH].
  - simpl. split.
    + intros H. apply I.
    + intros H. intros x. intros F. destruct F.
  - simpl. rewrite <- IH. split.
    + intros H. split.
      -- apply H. left. reflexivity.
      -- intros x H1. apply H. right. apply H1.
    + intros [H1 H2]. intros x. intros [H | H].
      -- rewrite <- H. apply H1.
      -- apply H2. apply H.
Qed. 
...
Lemma rev_append_plus' : forall X (l:list X) (l2: list X), 
  rev_append l l2 = rev_append l [] ++ l2.
Proof.
  intros X l.
  induction l as [ | x' l' IH].
  - reflexivity.
  - intros l2. simpl. rewrite IH. rewrite (IH [x']). rewrite <- app_assoc.
    reflexivity.
Qed.
Lemma rev_append_plus : forall X (l:list X) (x: X), 
  rev_append l [x] = rev_append l [] ++ [x].
Proof.
  intros X l.
  induction l as [ | x' l' IH].
  - reflexivity.
  - intros x. simpl. rewrite IH. rewrite (rev_append_plus' X l' [x'; x]).
...
Proof.
  ...
  induction l as [ | x' l' IHl'].
  - reflexivity.
  - simpl. rewrite <- IHl'. unfold tr_rev. simpl. apply rev_append_plus.
Qed.  
(** [] *)
...
Proof.
  ...
  - destruct (evenb (S n')) eqn: H.
    + rewrite evenb_S in H. 
      assert (H1: evenb n' = false). 
      { destruct (evenb n').
        - discriminate.
        - reflexivity.
      }
    ...
    + rewrite evenb_S in H.
      assert (H1: evenb n' = true).
      {destruct (evenb n'). -reflexivity. - discriminate. }
      rewrite H1 in IHn'. destruct IHn' as [k Hk].
...
Proof.
  ...
  - unfold not. intros H. destruct (x =? y) eqn: HE.
    + apply eqb_true in HE. apply H in HE. exfalso. apply HE.
    + reflexivity.
Qed.
(** [] *)
...
:=
  ...
  | x :: l' => match l2 with
               | [] => false
               | y :: l'' => if eqb x y then eqb_list eqb l' l'' else false
               end
  end.
...
Proof.
  intros A eqb. split. 
  - generalize dependent l2. induction l1 as [ | x l1' IH1].
    + intros l2. destruct l2 eqn:H2.
      -- intros H'. reflexivity.
      -- simpl . intros H'. discriminate H'.
    + intros l2. destruct l2 eqn:H2.
      -- simpl. intros H'. discriminate H'.
      -- destruct (eqb x x0) eqn:Heq.
         ++ simpl. rewrite Heq. intros H'. apply IH1 in H'.
            rewrite H'. apply H in Heq. rewrite -> Heq. reflexivity.
         ++ simpl. rewrite Heq. intros H'. discriminate H'.
  - intros H'. rewrite <- H'. generalize dependent l2. induction l1 as [| x l IHl].
    + intros l2. reflexivity.
    + intros l2. intros H'. simpl. assert (Hx : x = x). { reflexivity. }
      apply H in Hx. rewrite Hx. destruct l2 eqn: H2. 
      -- discriminate H'.
      -- injection H' as Hx' Hl'. apply IHl in Hl'. apply Hl'.
Qed.
(** [] *)
...
Proof.
  ...
  - induction l as [| x' l' IHl'].
    + reflexivity.
    + simpl. rewrite andb_true_iff. intros [Hx H].
      split.
      -- apply Hx.
      -- apply IHl' in H. apply H.
  - induction l as [| x' l' IHl'].
    + reflexivity.
    + simpl. intros [Hx H]. rewrite andb_true_iff. split.
      -- apply Hx.
      -- apply IHl' in H. apply H.
Qed.
...
Proof.
  ...
  assert (H1: P -> False). { intros p. 
    assert (H2: P \/ (P -> False)). { left. apply p. }
    apply H in H2. apply H2. }
  assert (H3: P \/ (P -> False)). { right. apply H1. }
...
Proof.
  unfold excluded_middle. intros H.
  intros X P HN x. destruct (H (P x)). 
  - apply H0.
  - exfalso. apply HN. exists x. apply H0.
Qed.
(** [] *)
...
Theorem pierce_implies_dne: 
  peirce -> double_negation_elimination.

Proof.
  unfold peirce. unfold double_negation_elimination.
  intros H. intros p. unfold not. intros H2.
  apply (H p False). intros H3. apply H2 in H3. exfalso. apply H3.
Qed.
  

Theorem dne_implies_em :
  double_negation_elimination -> excluded_middle.
Proof.
  unfold double_negation_elimination. unfold excluded_middle.
  intros dne. intros p. assert (H: ~~(p \/ ~p) ). { apply excluded_middle_irrefutable. }
  apply (dne (p \/ ~p)). apply H. Qed.

Theorem em_implies_ito :
  excluded_middle -> implies_to_or.

Proof.
  unfold excluded_middle. unfold implies_to_or.
  intros em. intros p q. intros H. assert (Hp : p \/ ~p).
  { apply em. } destruct Hp.
  - right. apply H. apply H0.
  - left. apply H0.
Qed.

Theorem em_implies_dne: excluded_middle -> double_negation_elimination.
Proof. intros em. intros P. destruct (em P) as [p|np].
  - intros nnp. apply p. 
  - intros nnp. exfalso. apply nnp. apply np. Qed.


Theorem ito_implies_em: 
  implies_to_or -> excluded_middle.
Proof. intros ito P. destruct (ito P P) as [H|H].
  intros p. apply p. right. apply H. left. apply H. Qed.  

Theorem ito_implies_dmnan: 
  implies_to_or -> de_morgan_not_and_not.
Proof. intros ito. 
  assert (dne: double_negation_elimination).
    {apply em_implies_dne. apply ito_implies_em. apply ito. }
  intros P Q H. 
  assert (HH: (~~P)\/Q -> P\/Q).
    {intros [nnp|q]. 
       + left. apply dne. apply nnp.  
       + right. apply q. }
  apply HH. apply (ito (~P) Q).
  intros notp. apply dne. intros notq. apply H. 
  split. apply notp. apply notq. Qed.
  
Theorem dmnan_implies_em: 
  de_morgan_not_and_not -> excluded_middle.
Proof. intros dmnan. intros P. apply (dmnan P (~P)).
  intros H. destruct H as [np nnp].
  apply nnp. apply np. Qed.

Theorem dne_implies_pierce: 
  double_negation_elimination -> peirce.
Proof. intros dne. intros P Q H. apply dne.
  unfold not. intros H2. apply H2, H. intros p.
  exfalso. apply H2. apply p. Qed.
